...
export default class Board {
  ...
  start() {
    this.frozen = false;
    this.handleSet(Jewel.retrieveMovedJewels());
  }
  ...
  updateInCols(posRow) {
    posRow.forEach((pos) => {
      ...
      this.cols[pos.x][0] = Jewel.random(new Pos(pos.x, 0), this.game.grid, this.level).place(700) // wait for swap and remove animation
    })
  }
  ...
  clear() {
    Jewel.retrieveMovedJewels(); // clear out last level's jewels
    setTimeout(() => {
      const allJewels = [];
      for (let y = 7; y >= 0; y--) {
        for (let x = 0; x < 8; x++) {
