...
export default class Board {
  ...
  populate() {
    for (let x = 0; x < 8; x++) {
      this.cols[x] = {};
      for (let y = 0; y < 8; y++) {
        this.cols[x][y] = Jewel.random(new Pos(x, y), this.container, this.level).place()
      }
    }
  }
  ...
  updateInCols(posRow) {
    posRow.forEach((pos) => {
      ...
      this.cols[pos.x][0] = Jewel.random(new Pos(pos.x, 0), this.container, this.level).place(500)
    })
  }
